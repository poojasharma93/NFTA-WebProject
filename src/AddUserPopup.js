...
class AddUserPopup extends Component {
  ...
  validateFields = e => {
    ...
    if (isValid) this.addUser();
  };

  async addUser() {
    console.log(
      "Here" + this.state.username,
      this.state.password,
    ...
    if (this.state.addUserResult.status === 401) {
      ...
      this.state.addUserResult.user_id !== undefined &&
      this.state.addUserResult.user_id !== ""
    ) {
      this.setState({
        message:
          "User added succesfully with ID: " + this.state.addUserResult.user_id
      });
